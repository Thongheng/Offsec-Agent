...
from engagement import engagement_root, scope_file  # noqa: E402
from scope_check import load_scope, check_target  # noqa: E402
...
def rate_limit_wait(scope_cfg: dict) -> None:
    ...
    lock = os.path.join(str(engagement_root()), ".lastrun")
    now = time.time()
    try:
...
def main() -> int:
    ...
    try:
        ...
        return 2
    verdicts = [check_target(cfg, t) for t in {target, host_of(host)}]
    for v in verdicts:
